...
import { appointmentIdSchema, loginDoctorSchema, updateDoctorSchema } from "@/types/doctor.types";
import z from "zod";
...
export async function appointmentComplete(req: Request, res: Response) {
  ...
  if (appointment.docId.toString() !== req.user!.id) throw new ForbiddenError("Not authorized")

  await Appointment.findByIdAndUpdate(result.data.appointmentId, { isCompleted: true })
